...
{
    public class PersianCultureInfo : System.Globalization.CultureInfo
    {
        private const BindingFlags AllModifiersBindingFlag =
        ...
        ///     A Boolean that denotes whether to use the user-selected culture settings (true) or the
        ///     default culture settings (false).
        /// </param>
        public PersianCultureInfo(bool useUserOverride)
                    : base(CultureInfoName)
        {
            AddPersianCalendar(this);
        }

        /// <summary>
        ///     Adds <see cref="PersianCalendar" /> to the list of optional calendars of the specified <paramref name="culture" />
        ///     in the given <paramref name="index" />.
        /// </summary>
        /// <param name="culture">
        ///     The <see cref="CultureInfo" /> instance to add the <see cref="PersianCalendar" /> to its list of
        ///     OptionalCalendars.
        /// </param>
        /// <param name="index">
        ///     The index of element in optional calendars to be set to PersianCalendar or a negative number to add
        ///     the PersianCalendar at the end of available calendars.
        /// </param>
        /// <returns>
        ///     Same instance of passed <paramref name="culture" /> which has <see cref="PersianCalendar" /> as an optional
        ///     calendar.
        /// </returns>
        private static System.Globalization.CultureInfo AddOptionalCalendars(System.Globalization.CultureInfo culture, int index)
        {
            Debug.Assert(culture != null, "culture is null");

            var cultureDataField = culture.GetType().GetTypeInfo().GetField("m_cultureData", AllModifiersBindingFlag);
            if (cultureDataField != null)
            {
                var cultureData = cultureDataField.GetValue(culture);
                var calendarIdProperty = cultureData.GetType().GetTypeInfo().GetProperty("CalendarIds", AllModifiersBindingFlag);
                var calendars = (int[])calendarIdProperty.GetValue(cultureData, null);

                if (index >= calendars.Length)
                    throw new ArgumentException(
                        string.Format("index should be less than {0} (Optional Calendars length).", calendars.Length));
        ...
        public static System.Globalization.CultureInfo AddPersianCalendar(System.Globalization.CultureInfo culture)
        {
            if (culture == null)
            ...
            var cultureInfoReadOnly = typeof(System.Globalization.CultureInfo).GetTypeInfo().GetField("m_isReadOnly", AllModifiersBindingFlag);
            ...
            {
                var cultureCalendar = typeof(System.Globalization.CultureInfo).GetTypeInfo().GetField("calendar", AllModifiersBindingFlag);
        ...
        /// <summary>
        ///     Localizes the specified <paramref name="dateTimeFormat" /> and adds an instance of <see cref="PersianCalendar" />
        ///     as its calendar.
        /// </summary>
        /// <param name="dateTimeFormat">The DateTimeFormatInfo to localize.</param>
        /// <returns>
        ///     Same instance of <see cref="DateTimeFormatInfo" /> passed but localized and with an instance of
        ///     <see cref="PersianCalendar" /> as its calendar.
        /// </returns>
        ...
        private static DateTimeFormatInfo LocalizeDateTimeFormatInfo(DateTimeFormatInfo dateTimeFormat)
        {
            ...
            var formatInfoIsReadOnly = infoType.GetTypeInfo().GetField("m_isReadOnly", AllModifiersBindingFlag);
            var formatInfoCalendar = infoType.GetTypeInfo().GetField("calendar", AllModifiersBindingFlag);

            if (formatInfoIsReadOnly == null)
                throw new ArgumentException("Unable to get IsReadOnly field from the specified DateTimeFormatInfo.",
                    "dateTimeFormat");

            if (formatInfoCalendar == null)
                throw new ArgumentException("Unable to get Calendar field from the specified DateTimeFormatInfo.",
                    "dateTimeFormat");
